...
import { MinimalisticMagnifier, AltArrowDown, ArrowRight, CheckRead, CheckSquare } from '@solar-icons/react';
...
export default function DevlogList({ allLogs, lang }) {
  ...
  return (
    <motion.div 
      ...
      exit={{ x: -50, opacity: 0 }}
      /* On garde tes valeurs exactes pour le desktop (md:)
         Et on ajoute le responsive uniquement pour le mobile 
      */
      className="fixed md:absolute inset-y-6 left-0 md:left-6 z-40 flex flex-col gap-5 pointer-events-none w-full md:w-[55svw] pt-16 md:pt-0 px-3 md:px-0"
    >
      {/* BARRE DE RECHERCHE */}
      <div className="p-3 bg-ui-bg rounded-3xl flex items-center gap-2 pointer-events-auto ">
      ...
      {/* LISTE DES PREVIEWS */}
      <div className="flex-1 bg-ui-bg rounded-3xl p-6 overflow-y-auto pointer-events-auto flex flex-col gap-4 scrollbar-hide">
        {filteredLogs.map(log => (
          <div key={log.id} onClick={() => navigate(`/devlog/${log.date}/${log.slug}`)} className="flex flex-col lg:flex-row gap-6 items-start hover:bg-white/5 p-4 rounded-2xl transition-all group cursor-pointer">
              <div className="w-full md:w-auto md:h-[270px] aspect-[9/16] bg-black rounded-xl overflow-hidden shrink-0 border border-white/10">
